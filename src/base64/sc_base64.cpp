...
int base64_encode(const char *ptr, int ptr_len, char *buf, int buf_len)
{
  ...
  if(remainder != 0)
  {
    ...
    *p_des       = base64_encode_map[(*buf >> 2) & 0x3f];
    *(p_des + 1) = base64_encode_map[((*buf << 4) & 0x30) | ((*(buf + 1) >> 4) & 0x0f)];
    *(p_des + 2) = base64_encode_map[(*(buf + 1) << 2 & 0x3c) | ((*(buf + 2) >> 6) & 0x03)];
    *(p_des + 3) = base64_encode_map[*(buf + 2) & 0x3f];
    p_src += remainder;
    p_des += 4;
  }
